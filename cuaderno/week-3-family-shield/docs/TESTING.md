...
| DEPLOY-02 | 2026-08-29 | Deployment 2, Vercel production, 390 x 844 px | Enter the organized summary and independent-verification screens after scrolling to each prior action | Every new screen starts at the top with the complete stop banner visible | The first production attempt exposed a delayed scroll-restoration edge on verification; the strengthened build reset immediately, on the next frame, and after 100 ms. Both transitions then passed on the public alias | Fixed and passed | `docs/assets/test-07-deployment2-public.jpg` and [Deployment 2](https://family-shield-week-3.vercel.app) | The unaccepted intermediate production build was superseded before closing Deployment 2 | Passed |
...
| Organized summary | The app has reorganized what the message claims; it has not authenticated Diego | The screen opened at the previous scroll position, leaving most of "NO PAGUES TODAVÍA" above the mobile viewport | High | Reset the viewport to the top whenever the flow changes screen | Added an immediate, next-frame, and 100 ms step-change scroll reset in `src/App.tsx` | Passed locally and on Deployment 2 at 390 x 844 px; the complete stop banner is now the first content after both major transitions |
...
| Independent contacts | I should call a number I already had, never one supplied in the suspicious message | The same retained-scroll defect could partially hide the stop banner on entry | High | Apply the same step-change scroll reset | Covered by the same correction; the public retest added the delayed reset after exposing a timing edge | Passed on Deployment 2; the stop banner and verification heading are fully visible on entry |
...
| Deployment 2 | https://family-shield-week-3.vercel.app | `e3c2be0` | Simulated, visibly labeled | 2026-08-29 | Persona correction accepted after both public mobile transitions showed the complete stop banner |
